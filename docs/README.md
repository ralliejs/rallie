...
> setState, getState和watchState支持深度设置，读取和监听状态值。例如调用socket.setState('foo.bar', value), socket.getState('foo.bar'), socket.watchState('foo.bar', callback)是对bus.state.foo.bar进行设值，读取和监听。initState和waitState仅只允许初始化和等待根状态，不支持深度状态
